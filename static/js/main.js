...
const pondChangePrice = FilePond.create(
  document.querySelector(".filepond-change-price"),
  {
    allowMultiple: false,
    maxFileSize: "30MB",
    maxFiles: 1,
...
async function uploadHandler(pond, url, modeId, controllerId, messageId) {
  ...
  console.log(document.getElementById(modeId).value);
  ...
  const resp = await apiFetch(url || "/api/kaufland_main/", {
    method: "POST",
    body: formData,
  });
  if (!resp) return;
  ...
  if (resp.ok) {
    if (
      messageId === "upload-message-delete" &&
      formData.get("mode") === "checker"
    ) {
    ...
    console.log(err);
    msg.textContent =
      err.error || err.detail || err.message || "Ошибка сервера";
  }
}

// НАВЕШИВАЕМ ОБРАБОТЧИКИ ТОЛЬКО ПОСЛЕ ТОГО, КАК FilePond создал переменные!
document.getElementById("upload-button-delete").onclick = () =>
  uploadHandler(
    pondDelete,
    "/api/kaufland_main/",
    "mode-select-delete",
...
document.getElementById("upload-button-change-price").onclick = () =>
  uploadHandler(
    pondChangePrice,
    "/api/kaufland_main/",
    "mode-select-price",
    "controller-select-price",
    "upload-message-change-price"
  );

document.getElementById("upload-button-upload").onclick = () =>
  uploadHandler(
    pondUpload,
    "/api/kaufland_main/upload_json/",
    "mode-select-upload",
    "controller-select-upload",
...
function startUploadSocket() {
  if (uploadSocket && uploadSocket.readyState === WebSocket.OPEN) {
    return; // уже подключен
  }

  const protocol = location.protocol === "https:" ? "wss://" : "ws://";
  const socketUrl = protocol + location.host + "/ws/upload-progress/";

  uploadSocket = new WebSocket(socketUrl);

  uploadSocket.onopen = () => {
    console.log("✅ Upload progress socket connected");
  };
  ...
  uploadSocket.onclose = () => {
    console.warn("⚠️ Upload socket closed");
  };
}
...
function showProgress(data) {
  ...
  line.textContent = `EAN ${data.ean} → ${data.stage}`;
